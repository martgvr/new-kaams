		...
		<div className="checkoutform__container">
			<Formik
				...
				validate={(values) => {
					...
					else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(values.mail)) { errors.mail = "El correo debe tener el formato tu@email.com" }

					if (!values.telephone) { errors.telephone = true } 
					...
					else if (!/^[A-Z0-9\s]{1,40} [0-9\b]{1,6}$/i.test(values.address)) { errors.address = "La dirección debe tener el formato Calle Altura" }

					if (!values.city) { errors.city = true }
					else if (!/^[A-Z0-9\s]{1,40}$/i.test(values.city)) { errors.city = "La ciudad solo puede contener letras y números" }

					if (!values.neighborhood) { errors.neighborhood = true } 
					else if (!/^[A-Z0-9\s]{1,40}$/i.test(values.neighborhood)) { errors.neighborhood = "El barrio solo puede contener letras y números" }
							...
							<div className="form__box">
								...
								<div>
									...
									<input
										...
										onBlur={handleBlur}
										style={{
											border: errors.neighborhood && touched.neighborhood ? "1px solid red" : "" || (!errors.neighborhood && touched.neighborhood) ? "1px solid green" : "",
										}}
									/>
